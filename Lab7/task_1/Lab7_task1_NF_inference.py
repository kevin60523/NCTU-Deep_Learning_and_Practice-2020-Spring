...
model.load_state_dict(torch.load("./nf_0.36.pt"))
model.to(device)
model.eval()
...
for i in range(1000):
    with torch.no_grad():
        for labels in test_loader:
            ...
            noise = torch.randn(32, 3, 64, 64).to(device)
            z, sldj = model(noise, labels, reverse=True)
            score = model_E.eval(z, labels) 
            if best < score:
                best = score
                save_image(z.data, './bbtest.png', nrow=8, normalize=True)
print(best)